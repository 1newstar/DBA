...
def execute_insert(b,s):
	...
	col7=s
	sql="insert into binlogtosql values (" + col1 + ",'" + col2 + "','" + col3 + "'," + col4 + ",'" + col5 + "','" + col6 + '''',"''' + col7 + '''");'''
...
sql='create table binlogtosql (id int primary key auto_increment,edate date not null,etime time not null,pos int not null,event text not null,type varchar(20) not null,sqlinfo text not null) charset utf8;'
p = mysqlhelper(url,username,password,dbname )
p.createtable(sql)
p.close()
...
for j in range(1,num+1):
	a1=names['b_list%d'%j]
		...
		a2=p1.change_col(sql_list)
                ...
                a2='begin;'
	elif names['b_list%d'%j][3]=='commit':
		a2='commit;'
	else:
		a2='others'
